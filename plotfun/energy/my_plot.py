...
## Constants
# TODO (maybe) add all these constant to a dictionary.
...
def _calc_lin_int(x, min_x, max_x, r_min, r_max):
    """
    Calculates linear interpolation between the given values.
...
def plot_energymix(ax, val, max_data = df_energymix.sum(axis=1).max(),
    ...
    # Calculate the data values between the years to let the plot appear smooth.
    ...
    circle_size = _calc_lin_int(data.sum(), 0, max_data, 0, max_size)
...
df_wind_min = df_wind['Bruttoleistung'].min()
df_wind_max = df_wind['Bruttoleistung'].max()
...
def plot_map(ax, val,
    ...
    if not enable_before_val:
        ...
        # This is to try out different colors as power plants.
        ...
        c2 = color_book['colors_power_plants']['Windenergie']
        cmap = mc.LinearSegmentedColormap.from_list('mycmap', [np.asarray(ImageColor.getcolor(c2, 'RGB'))*0.1/255,c2])
        cbar = gp['fig'].colorbar(mpl.cm.ScalarMappable(cmap=cmap), cax=axins, ticks=[0,1])
        cbar.ax.yaxis.set_ticks_position('left')
        cbar.ax.set_yticklabels(['35kW', '12250kW'])
    ...
    if inter_map.empty:
        ...
        alpha_list = _calc_lin_int(inter_map['Bruttoleistung'],
                                   df_wind_min, df_wind_max , 0.3, 1.)

        s_list = [DOT_MARKERSIZE]*len(inter_map)
        ax.scatter(points.x, points.y, marker='o',
                   color=inter_map['color'], alpha=alpha_list,
                   zorder=2, s=s_list,
                   **plot_args_intermap)
...
def image_make(year, save_name = None):
    ...
    gp.plot_frame(dict(year=year, alpha=0.),
                  {'energymix' : dict(max_size=INNER_CIRCLE_RADIUS,
                    ...
                    'map' : dict(enable_before_val=False, gp=gp),
                    },
...
def animation_make(year_range, save_name = None, approx=10):
    ...
    anim=gp.plot_animation(frames,
                           fargs=({'energymix':dict(max_size=INNER_CIRCLE_RADIUS,
                                   ...
                                   'co2':dict(max_size=INNER_CIRCLE_RADIUS,
                                               max_data=df_co2.sum(axis=1).max()),
                                   'map':dict(enable_before_val=True)},),
