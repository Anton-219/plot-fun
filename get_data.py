...
def _parse_wind_turbines(needed_cols=None, color_book=None,
    ...
    if needed_cols is None:
        needed_cols = ['geometry', 'opening date', 'energy source']
...
def _parse_poly(path=None, needed_columns=None):
    ...
    needed_columns is a list describing the columns which stay in the dataframe.
...
def parse_data_marktstammdatenregister(path, needed_columns=None, to_save=False, s_path = None,
               driver='GeoJSON'):
    """
    ...
    if needed_columns is None:
        needed_columns = ['EinheitMastrNummer', 'Landkreis', 'Gemeinde', 'Postleitzahl', 'Ort', 'Laengengrad',
...
if __name__ == '__main__':
    ...
    dff = parse_data_marktstammdatenregister(wpath)
